from ranking.pagerank_biased import Ranking
...
class CFUR:
	...
	def __init__(self, default_path='citation-web/matrix-way/', file_='citation-matrix.csv', 
					file_pair=None, normalize_similarity=True):
	...
	def __normalize_rank__(self):
		...
		scores = Ranking(file_=self.default_path+self.file_pair, prior_bias_node=self.df.index.values).scores()
	...
	def recommend(self, index, n=10, k=20):
		...
		unknowns = set()
		already_known = set(self.df.loc[index][self.df.loc[index][self.column_names] == 1].index.values)
