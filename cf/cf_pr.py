from ranking.pagerank_biased import Ranking as BRanking
from ranking.pagerank import Ranking
...
class CFUPR:
	...
	def __init__(self, default_path='citation-web/matrix-way/', file_='citation-matrix.csv', 
					file_pair=None, normalize_similarity=True, biased=False):
	...
	def __normalize_rank__(self):
		...
		if self.biased:
			scores = BRanking(file_=self.default_path+self.file_pair, prior_bias_node=self.df.index.values).scores()
		else:
			scores = Ranking(file_=self.default_path+self.file_pair).scores()

		for index in tqdm(self.df.index.values):
			self.df.loc[index] = self.df.loc[index] * scores[index]
