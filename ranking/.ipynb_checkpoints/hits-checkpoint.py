...
class Ranking:
	...
	def __init__(self, file_, root_nodes=[], iterations=100, 
		...
		self.potential_hub_nodes = set(self.__potential_hubs__()) \
									-	self.root_nodes
		self.__base__()
	...
	def __calculate__(self, normalize):
		for _ in range(self.iterations):
			temp = {}
			for key in self.data.keys():
				temp[key] = sum([self.auth_score[i] for i in self.data[key]])

			self.auth_score = temp.copy()

	def scores(self, normalize=True):
		scores = {}
		self.__calculate__()
